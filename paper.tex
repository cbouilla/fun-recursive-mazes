...
\author{Orel Cosseron}{...}{email orel}{}{}
...
\keywords{Quantum computing, blockchains, IA, pushdown automaton, reachability, graph rewriting}
...
\begin{abstract}
  A recursive maze is a maze that contains a one or several copies of itself
  (that themselves contain one or several copies of themselves, etc.). To exit
  ...
  well-known to be decidable, and there is a well-known, ``classical'' algorithm
  for this task. We present a new algorithm, \textsc{Midterm}, with improved
  complexity compared to existing solutions.
...
A \emph{recursive maze} is a maze that contains copies of itself. They have
often been called ``fractal mazes'' on the internet. Because a picture is worth
a thousand words, the concept is better illustrated by Figures~\ref{fig:jpwolf}
and~\ref{fig:later}. Recursive mazes are a notoriously difficult kind of puzzle
to solve by hand, without taking notes and/or using an \textit{ad hoc} computer
program to track one's position in the maze. Several such program exist, but we
mention for instance \url{https://amurielagi.github.io/fractal-maze/}.

\begin{figure}
  \includegraphics[width=0.59\textwidth]{FractalMaze95.png}
  \includegraphics[width=0.39\textwidth]{fractal1.png}
  \caption{Two early recursive mazes designed by Mark J. P. Wolf. First published in Extropy \#17, Fall 1996, pages 66-67 (left) and in the "Bogglers" column of the June 2004 issue of Discover Magazine, page 86 (right). See also the book~\cite{WolfBook} by the same author. \label{fig:jpwolf}}
\end{figure}
...
\begin{figure}
  \begin{multicols}{2}
  \includegraphics[width=0.48\textwidth]{infinite_descent.pdf}
  \columnbreak
%\hfill
  \includegraphics[width=0.48\textwidth]{first_cantor.pdf}
\end{multicols}
\caption{Two unsolvable recursive mazes by Tristan ``Siggy'' Miller (published
  ...
  \href{https://freethoughtblogs.com/atrivialknot/2023/10/21/infinite-fractal-mazes/}{Freethought
    blogs}). On the right, the goal is to reach the ``F''. These mazes are
  unsolvable with the normal rules, but what if one allows transfinite
  moves? \label{fig:infinite}}
\end{figure}
...
``innocent-looking'' unescapable maze shown above.
...
\hfill
  ...
  \caption{A simple maze of our own design (left) and its ``dual'' representation (right). \label{fig:simple}}
\end{figure}
...
$\bigO{\mathcal{W}^3}$. Finally, in section~\ref{sec:us} we present a new
deterministic algorithm that terminates in time $\bigO{WB + E \log E}$,
improving the state of the art. Doing this in linear time is posed as an
...
description of the maze. Note that in principle each outer IO pin must appear in
one and only one wire for the maze to be well-formed.

However, an alternate, ``dual'' description is also useful
(Fig.~\ref{fig:simple}, right): it is a directed graph whose nodes are the wires
of the maze, and where there is a directed edge $\alpha \xrightarrow{X.i} \beta$
if it is possible to go from wire $\alpha$ to wire $\beta$ by entering the
recursive box labelled $X$ using its pin labelled $i$. We denote by $E$ the
number of edges of this graph. The dual-representation graph can be assembled in
essentially linear time from the previous ``hypergraph'' representation:
\begin{itemize}
...
where the player starts. The position of the player is made of a current wire
...
identifier of the ``main'' box that contains everything, which is denoted by
...
As a consequence, the sequences of instruction that exit the maze form a
...
\textsf{PDAAAL}~\cite{DBLP:conf/atva/JensenSSS22} (to just mention a few) that
...
\begin{center}
\begin{tikzpicture}[scale=1.25, every edge/.style={shorten >=0.5mm, draw}]
...
However, there is going to be a severe restriction. The regular language in the
intersection must be such that the graph describing the associated finite
automaton is ``undirected'' (see~\cite{DBLP:journals/acta/KutribMS22} for more
results on this kind of automata---they are said to be ``dual and
connected'' in~\cite{STEINBERG2002341}). This highlight a nasty ``feature'' of
recursive mazes: all moves are reversible. Mistakes can be undone without having
to hit CTRL+Z, just by entering an appropriate instruction (exit the box that
has just been entered, or re-enter the box that has just been left).
...
\subsection{Efficient deterministic implementation}
...
however navigate around two problems: first, the same pair of wires can be
merged by two (or more) wormholes (as shown in Fig.~\ref{fig:wormholes-pb},
left). In our implementation, the pending list may contain several wormholes
merging the same pair of nodes. To avoid potential issues, when processing the
next wormhole, we first test if the two wires have already been merged. The
second potential problem is that the number of wormholes can be quadratic in the
size of the maze (see Fig.~\ref{fig:wormholes-pb}, right). However, not all of
them can be collapsed: some will vanish into thin air when \emph{others} are
collapsed.  In fact, when a wormhole
$\alpha \xrightarrow{X.i} \beta \xleftarrow {X.j} \gamma$ is discovered and
added to the list, we immediately remove one of the two edges from the
graph---they would become identical later when the wormhole actually gets
contracted and one of them would be removed anyway, so why delay the inevitable?
With this modification, only $E$ wormholes can be added to the list. We
implement this as follows: we look at each edge of the initial graph in
...
supernodes could have been a once-in-a-lifetime chance for us to use the
celebrated ``inverse-Ackermann'' disjoint-set data structure (also known as
\textsc{Union-Find}). But unfortunately, or fortunately, we make do with a
simpler solution: we eagerly update all items when two sets are joined. We
nevertheless use several ideas from the original analysis.

Inside each supernode, one node is chosen as ``supernode leader''. The supernode
is hosted by the leader node in the original graph. Nodes that
have been merged into larger supernode are not actually removed from the graph ;
they remain there but become \emph{followers} (as opposed to their leader). All
nodes have a pointer (\texttt{leader}) to the leader node in their supernode
...
and (super)nodes are merged. %More precisely:
% \begin{enumerate}
% \item Each (super)node has a linked list of outgoing edges and incoming edges.
% \item If $\omega$ is the leader of a (super)node, then $\texttt{size}[\omega]$
%   is the size of the (super)node.
% \end{enumerate}

When two (super)nodes are merged, to form the resulting supernode, the smaller
one (by \texttt{size}) is ``absorbed'' into the larger one. More precisely:
\begin{itemize}
...
process, at least not until an actual exit path has to be constructed.
Doing all the work on the smaller of the two nodes minimizes the total amount of
work (this idea is common ; for instance it is at work in Hopcroft's DFA
minimization algorithm~\cite{hopcroft1971nlogn}).

The actual issue is to detect the new wormholes that are created by the
displacement of edges. We propose a straightforward and deterministic
solution that uses an array \texttt{hot} of dimension $W \times B$ such that
$\texttt{hot}[\beta,X]$ is either \texttt{none} or $\texttt{some} (\alpha, i)$,
in which case there is a ``hot'' edge $\alpha \xrightarrow{X.i} \beta$ in the
graph. The downside is that initializing the \texttt{hot} array could
potentially dominate the whole running time, at least asymptotically.

Because we need to efficiently iterate on the hot incoming edges to a node, we
also use an auxiliary array \texttt{hotbox} where $\texttt{hotbox}[\omega]$
contains a list of $X$'s such that $\texttt{hot}[\omega, X] \neq
\texttt{none}$. This is a lightweight variant of the ``sparse array'' technique
abundantly used in sparse matrix codes and presented
in~\cite{10.1145/176454.176484}, in a different context.

The pseudo-code of our solution in shown as Algorithm~\ref{algo:main}. We
believe that it is simple enough to be implemented in assembler by an
...
determined. This easily allows to test if the maze can be exited. The actual
construction of an exiting path is deferred to section~\ref{sec:arg}.
...
\begin{algorithmic}[1]
  ...
  \Procedure{AddEdge}{$\alpha, X, i, \beta$}
   ...
   \State $\texttt{hotbox}[\beta] \gets X :: \texttt{hotbox}[\beta]$
   \EndIf
   \EndProcedure
  ...
  \For{each $X \in \texttt{hotbox}[\sigma]$}                           
  ...
  \Procedure{CollapseMaze}{$W, B, \texttt{G}$}                          
  ...
  \For{$0 \leq \alpha < W$}
    ...
    \For{$0 \leq X < B$}
    \State $\texttt{hot}[\alpha,X] \gets \texttt{none}$                 \Comment{potential ``hot-spot''}
    \EndFor
  ...
  \EndProcedure
\end{algorithmic}
\caption{Algorithm to collapse recursive mazes. \label{algo:main}}
\end{algorithm}
...
\begin{center}
  ...
  \begin{tikzpicture}[scale=1]       % STEP 0
    \node[draw, fill=orange] at (4, 2) (start) {$\alpha_1$};
    \node[draw,fill=Orchid,text=white] at (2, 2) (K) {$\beta_2$};
    \node[draw,fill=red,text=white] at (0, 2) (R) {$\gamma_4$};
    \node[draw,fill=LimeGreen,text=white] at (0, 0) (G) {$\zeta_1$};
    \node[draw,fill=Aquamarine,text=white] at (2, 0) (B) {$\delta_3$};
...
% not hot    \draw (G) edge[->] node[left] {$B$} (R);
  ...
  \begin{tikzpicture}[scale=1]      % STEP 1
    \node[draw, fill=orange] at (4, 2) (start) {$\alpha_1$};
    \node[draw,fill=Orchid,text=white] at (2, 2) (K) {$\beta_3$};
    \node[draw,fill=red,text=white] at (0, 2) (R) {$\gamma_4$};
  ...
  \begin{tikzpicture}[scale=1]      % STEP 2
    \node[draw, fill=orange] at (4, 2) (start) {$\alpha_1$};
    \node[draw,fill=Orchid,text=white] at (2, 2) (K) {$\beta_3$};
    \node[draw,fill=red,text=white] at (0, 2) (R) {$\gamma_6$};
    \node[draw,semitransparent,fill=LimeGreen,text=white] at (0, 0) (G) {$\zeta$};
  ...
  \begin{tikzpicture}[scale=1]      % STEP 3
    ...
    \node[draw,fill=red,text=white] at (0, 2) (R) {$\gamma_{7}$};
    \node[draw,semitransparent,fill=LimeGreen,text=white] at (0, 0) (G) {$\zeta$};
    \node[draw,semitransparent,fill=Aquamarine,text=white] at (2, 0) (B) {$\delta$};
  ...
  \begin{tikzpicture}[scale=1]      % STEP 4
    ...
    \node[draw,semitransparent,fill=Orchid,text=white] at (2, 2) (K) {$\beta$};
    \node[draw,fill=red,text=white] at (0, 2) (R) {$\gamma_{8}$};
    \node[draw,semitransparent,fill=LimeGreen,text=white] at (0, 0) (G) {$\zeta$};
    \node[draw,semitransparent,fill=Aquamarine,text=white] at (2, 0) (B) {$\delta$};
...
\caption{Step-by-step illustration of Algorithm~\ref{algo:main}. The plain edges
  are ``hot''. The dotted edges represent the \texttt{leader} array. The
  \texttt{size} of each (super)node is indicated in
  subscripts. \label{fig:step-by-step}}
\end{figure}

\subsection{Complexity}

The setup phase of \textsc{CollapseMaze} requires $\bigO{WB}$ operations. It is
clear that \textsc{AddEdge} runs in constant time, and that \textsc{Merge} runs
in time dominated by the size of the smaller supernode: it takes constant-time
per wire in the smaller supernode, plus constant time per hot edge pointing to
the smaller supernode (thanks to the \texttt{hotbox} array)---and not all edges
are hot, hence the upper-bound.
...
The number of iterations of the \textbf{while} loop is at most $E$ (each new
wormhole added to \texttt{pending} destroys an edge). It remains to control the
total time spent in \textsc{Merge}---this is the tricky part. As the algorithm
progresses, the sequence of merges can be represented by a binary forest. The
wires of the maze are the leaves, and each call to \textsc{Merge} creates a new
root whose two children are preexisting roots. It represents a new supernode
containing all the wires associated to the leaves in the subtree. Each supernode
is labelled by its size (as defined above). When two roots of labels $x \leq y$
are joined, the result has label $x + y$, and we are ``charged'' $x$ operations
(the smaller size).
...
Let $\mathcal{F}$ denote an arbitrary merge forest built from the
dual representation graph of a recursive maze. Its $W$ leaves are labelled with
the size of the wires (one plus the number of incoming hot edges). Let $S$
denote the sum of sizes: if the dual representation graph has $E$ edges, then
...
Therefore, the total running time of \textsc{CollapseMaze} is
$\bigO{BW + E \log E}$. It is unfortunate that the running time could
potentially be dominated by the initialization of an empty array, but here we
are. When the dual representation graph is dense ($S \approx W^2$), this yields
a complexity of order $W^2 \log W$, that slightly improves upon preexisting
techniques. However, when the graph is sparse and $S \approx W$, then the $BW$
term likely dominates, and under the plausible assumption that $W \approx B$,
then this gives a quadratic algorithm.

This analysis has a somewhat obvious shortcoming: the number of hot edges is at
most $BW$, and this could be smaller than $E$ if the graph is dense and
$B \ll W$. In this case, setting the \texttt{size}'s to the number of wires in a
supernode would result in a algorithm of running time $\bigO{WB + W \log W}$
(there are at most $W$ merges, and each one spend time at most $B$ processing
hot edges). Setting the \texttt{size}'s to the number of wires plus the number
of incoming hot edges would likely reduce the actual running time, but it seems
that it breaks the $E \log E$ argument.

To suppress the $\bigO{WB}$ term corresponding to the initialization of
\texttt{hot}, one possibility would be to use a hash table instead of a
bi-dimensional array. However, this then requires some kind of randomization and
we leave it as an open problem.

\subsection{Construction of an exit path}
\label{sec:arg}
...
Algorithm~\ref{algo:main} to provide the computation of such paths without extra
cost.
...
$\alpha$ to wire $\beta$, while entering box $X$.


So, before the graph rewriting process even start, we transform the dual
representation graph into the \emph{extended dual representation graph} shown in
Fig.~\ref{fig:extended}. We then perform the same graph rewriting process,
except that we now take care of instruction sequence identifiers as shown in
...
sequence of instruction $\# \mathcal{K} := \mathcal{I} ; \overline{\mathcal{git push -u origin mainJ}}$
...
$\overline{\# \mathcal{K}}$ must be added before their instruction sequence. We finally record the ``spanning'' edge $\alpha--\gamma$ with instruction sequence identifier $\# \mathcal{K}$.

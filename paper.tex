...
\keywords{Quantum computing, blockchains, IA, pushdown automaton, reachability, graph rewriting, recursive maze, $(\lambda x. xx)(\lambda x. xx)$}
...
program to track one's position in the maze. Several such program exist, such as
\begin{center}
...
deterministic algorithm that terminates in time $\bigO{WB + E \alpha(W)}$, where
$\alpha$ is the inverse-Ackermann function, improving the state of the art. We
discuss how to bring this down to $\bigO{E \log E}$, but this seems to either
require: a) cheating, or b) randomization.

Doing this in linear time, e.g. $\bigO{W + B + E}$ is posed as an intriguing
open problem.
...
maze of Fig.~\ref{fig:simple}, $(1, 2)$ and $(3, 5)$ are connected. But is this
really necessary? When two outer IO pins belong to the same wire, couldn't we
remove one of them (and redirect connections to the other one)? To answer this
question, take the dual representation graph and \emph{erase} all pin
information:
...
\item Different wires are not ``inadvertently'' connected to the same outer IO
  pins. This means that $\{i\}, \{j, n, o\}$, $\{p, m, k\}$ must be pairwise disjoint.
\item Labels must be unique. This means that inside the following sets, values are all different: $\{i, \ell\}, \{j, o\}, \{p, k\}$ and $\{n, m\}$.
\end{itemize}
...
\begin{tikzpicture}[xscale=1.33]
  \node[wire,fill=orange] at (0, 0) (alpha) {$\alpha$};
  \draw (alpha) edge[loop below] node[below] {$A.1$} ();
  
  \node[minimum size=6mm, draw] at (1, -2) (upsilon) {};
  \node[wire, draw] at (1, -2) {$\upsilon$};
  ...
  \draw (epsilon) edge[->] node[above] {$B.3$} (delta);
  \draw (zeta) edge[->] node[above] {$B.4$} (epsilon);
  ...
  \draw (lambda) edge[->] node[above] {$C.6$} (kappa);
  \draw (kappa) edge[->]  node[above] {$C.5$} (iota);
  \draw (iota) edge[->]   node[above] {$C.4$} (theta);
  \draw (theta) edge[->]  node[above] {$C.3$} (eta);
  ...
  \draw (xi) edge[->] node[below] {$D.4$} (nu);
  \draw (nu) edge[->] node[below] {$D.3$} (mu);
  ...
  \draw (omicron) edge[->] node[above,very near start] {$D.5$} (alpha);
  \draw (pi) edge[->] node[above] {$E.2$} (omicron);
  \draw (rho) edge[->] node[above] {$E.3$} (pi);
  \draw (sigma) edge[->] node[above] {$E.4$} (rho);
  \draw (tau) edge[->] node[above] {$E.5$} (sigma);
  \draw (tau) edge[->, bend left] node[below=0mm, near start] {$F.1$} (alpha);
\end{tikzpicture}
\end{center}

The resulting maze has 6 pins, 6 recursive boxes and 20 wires.
% Starting from $\alpha$, the self-loop enables entering arbitrarily many times
% the $A$ box (in 1 move), so entering $A$ costs 1. When $A$ has been entered 2
% times, it is possible to land on wire $\alpha$ inside box $B$, in 5 moves (so
% $B$ represents 7). Once $B$ has been entered 3 times, we can land on wire
% $\alpha$ inside box $C$, in 19 moves in total (so $C$ represents 19). Etc..
An exit path is provided by:
\begin{align*}
  \mathcal{P}_5    &:= A.1 ; A.1; \overline{A.2}; \overline{A.3}; B.1 \\
  \mathcal{P}_{19}  &:= \left(\mathcal{P}_5\right)^3 ; \overline{B.2}; \overline{B.3};  \overline{B.4}; C.1 \\
  \mathcal{P}_{101} &:= \left(\mathcal{P}_{19}\right)^5 ; \overline{C.2}; \overline{C.3};  \overline{C.4}; \overline{C.5};  \overline{C.6}; D.1 \\
  \mathcal{P}_{307} &:= \left(\mathcal{P}_{101}\right)^3 ; \overline{D.2}; \overline{D.3};  \overline{D.4}; E.1 \\
  \mathcal{P}_{1335}&:= \left(\mathcal{P}_{101}\right); \left(\mathcal{P}_{307}\right)^4; \overline{E.2};  \overline{E.3}; \overline{E.4};  \overline{E.5}; F.1 \\
  \mathcal{P}_{1337}&:= \left(\mathcal{P}_{1335}\right); \overline{F.2}; \texttt{done}
\end{align*}
It is left to the reader to check that $\mathcal{P}_x$ has length $x$. We also
hope that, at this point, the role of the ``twisted addition chains'' has become
clear. A Twisted addition chain is a sequence of equalities of the kind
$a_{i+1} = [\text{sum of $k$ previous $a_i$'s}] + (k+1)$, starting with
$a_0 = 1$. The number of pins of the maze is the maximum number of occurrences
of any given value in the twisted addition chain. The number of recursive boxes
is the number of steps of the twisted addition chain. The number of wires is two
plus the total number of occurrences of all constants in right-hand sides of the
twisted addition chain.
...
\end{align*}
leads to a different maze with 8 recursive boxes, 5 pins and only 19
wires. There is a rich theory of normal addition chains (see~\cite{Knuth}), and
the twisted ones beg for the same level of attention.
...
Looking forward, we see many interesting problems related to awaiting for a solution, 

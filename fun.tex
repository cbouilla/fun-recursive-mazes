...
% this is the "short" 20-pages version
...
% some figures which are not in the figure-environment which should be
% changed. Unfortunately, the formatting creates spaces of more than one page
% between the reference and the referenced figure, even for its first usage,
% e.g., Figure 6.

\EventEditors{John Iacono}
\EventNoEds{1}
\EventLongTitle{13th International Conference on Fun with Algorithms (FUN 2026)}
\EventShortTitle{FUN 2026}
\EventAcronym{FUN}
\EventYear{2026}
\EventDate{May 18--22, 2026}
\EventLocation{Porquerolles, France}
\EventLogo{}
\SeriesVolume{366}
\ArticleNo{12}
...
\relatedversiondetails{Extended full version}{https://inria.hal.science/view/index/docid/5517329}

\keywords{Recursive maze, pushdown automaton, reachability, context-free grammar, graph rewriting}

\acknowledgements{We thank Ambroise Fleury for involuntarily suggesting this
  work, and the anonymous reviewers for their constructive feedback as well as
  for spotting many, many typos.}
...
have been invented by Mark J. P. Wolf and the first one was published in 1996
under the name ``\emph{fractal maze}'' (\emph{Extropy} \#17, p. 67, available in
the \href{https://archive.org/details/extropy-17/page/66/mode/2up}{internet
...
maze. Several such program exist, notably to play in a web browser\footnote{For instance \url{https://amurielagi.github.io/fractal-maze/} or [shameless advertisement] \url{https://orelcosseron.github.io/fractal-maze/}.}.
...
one of its outer IO pin. This is where the real fun begins. For one, exiting may
not be possible at all. Consider for instance the innocent-looking maze:
\begin{center}
\begin{tikzpicture}
  % special
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.5\textwidth]{power4.pdf}
...
make the conversion the other way around.
...
\begin{figure}[t]
  ...
  \draw[very thick, LimeGreen] (8.5, 7) -- (8.5, 8) -- (6, 8) -- (6, 4) -- (6.5, 4) node[right] {$\epsilon$} -- (6.5, 3);
...
\begin{tikzpicture}[scale=1.25]
  ...
  \node[wire,fill=LimeGreen,text=white] at (0, 0) (G) {$\epsilon$};
  ...
  \node[wire,fill=Orchid,text=white] at (2, 2) {$\beta$};
    ...
    \draw (B) edge[->, loop right] node[right] {$C.5$} (C);
    \draw (B) edge[->, bend left] node[right] {$A.2$} (R);
    \draw (G) edge[->] node[below] {$D.3$} (B);

    \draw (R) edge[->, loop left] node[left] {$D.1$} (R);
...
``Solving'' recursive mazes in linear time, e.g. $\bigO{W + E + B}$ is left as
...
\overline{A.4}$. Applying a sequence of legal instructions then its mirror is a
no-op.

There is a way to efficiently check if a sequence of instructions is valid,
i.e. if the desired moves are actually feasible. The validity of a move depends
...
$\displaystyle
  ...
  \xrightarrow{\overline{B.2}}   \frac{\epsilon  }{ A \bot}
...
$[\alpha X \beta]$ is the set of sequences of instructions that move from wire
...
\end{itemize}
\noindent Then for each edge $\phi \xrightarrow{X.i} \psi$ in the dual representation of
the maze:
\begin{itemize}
...
There is a more efficient technique that can be better suited to
laaaaaaaaaaaarge recursive mazes. It is heavily inspired by algorithms designed
almost simultaneously by Bouajjani, Esparza and
...
algorithms decide reachability properties in pushdown systems, in the context of
model checking problems that could admit such a representation.
...
The idea is that there is a sequence of instructions that takes the player from
...
\begin{center}
  \begin{tikzpicture}[scale=1.25]
    ...
    \node[anchor=west] at (5, 1.75) {$\beta \xrightarrow{B.1} \gamma \xleftarrow{B.2} \epsilon$};
    \node[anchor=west] at (5, 1) {$\gamma \xrightarrow{C.3} \delta \xleftarrow{C.5} \delta$};
    \node[anchor=west] at (5, 0.25) {$\gamma \xrightarrow{D.1} \gamma \equiv \delta \xleftarrow{D.3} \epsilon$};
  \end{tikzpicture}
\end{center}

Note that the wormhole between $\gamma$ and $\epsilon$ is transitive
($\gamma \not\equiv \epsilon$ but $\gamma \stackrel{*}{\equiv} \epsilon$). From this,
we observe that
...
labelled with a length. Therefore, not only it is possible to find a shortest
path from the starting point to an exit (using only wormhole edges), but it is
possible to emit a description of this path in polynomial time, even though the
path may have exponential length, by printing the labels of the crossed wormhole
edges.
...
On this occasion, we cannot resist the urge of a little digression.  We see a
striking analogy with abstract quantum computational models: applying a unitary
transformation to the state of a quantum system is reversible, \emph{just like
  moves in a recursive maze}. This sheds new light to the intervention of Peter
Shor mentioned in section~\ref{sec:pda}---we are not so naive as to believe that
it is a mere coincidence. Therefore, we plan to push forward the idea that
recursive mazes are ideally suited as a computational model for future quantum
computers (although we are going to leave that aspect aside for future work).

Back to the problem at hand, this makes it possible for us, instead of
explicitly building a clique between equivalent wires as
...
After a draft of this paper was written, we discovered that the same operation
has been called ``folding''
in~\cite{Stallings1983,b97de931-5994-30c4-b01e-c43e12a3f8e6,STEINBERG2002341},
in quite a different context---profinite topologies on a free group, if you can
believe it. The main properties of this process have already been proved in
these works, and because of space constraints we will not repeat them: wormholes
can be contracted in any order, and at the end we get exactly the equivalence
classes of $\stackrel{*}{\equiv}$. The intuition behind this is that contracting
a wormhole may create new wormholes but not destroy preexisting ones (other than
the one being contracted, of course). This is because a wormhole requires two
similarly-labelled edges pointing to the same node ; contracting a wormhole can
only bring together the endpoints of edges previously pointing to different
nodes, not take them apart.
...
Starting from the dual representation graph of a maze, we run the contraction
process until death ensues. To check if it is possible to exit the maze, it is
sufficient to examine the wires in the equivalence class of the starting point,
and test if one of them is connected to an outer IO pin. When it is the case, an
exit path can be built explicitly (but how to do so depends on implementation
...
constant time. We make abundant use of linked lists, and we
...
merged and only proceed when it is not the case (this solves the first
problem). In addition, when a wormhole
...
the wormhole and the edge under scrutiny is sucked into the void), or
it does not (we mark the edge as ``hot'').
...
When two (super)nodes are merged, one of them is in fact ``absorbed'' into the
other one. Hot edges pointing to the absorbee either create a wormhole and are
dropped, or become hot edges pointing to the absorber.  The actual issue is to
detect the new wormholes that are created by the displacement of edges.
...
supernodes turns out to be once-in-a-lifetime chance for us to use the
celebrated ``inverse-Ackermann'' disjoint-set data structure (also known as
\textsc{Union-Find}). This uses the two arrays \texttt{parent} and
\texttt{rank}.
...
undergrad. Its step-by-step execution on the simple maze of
...
\begin{figure}
\begin{center}
  \begin{tikzpicture}[scale=0.93]       % initial graph
    ...
    \node[draw,fill=LimeGreen,text=white] at (0, 0) (G) {$\epsilon$};
  ...
  \begin{tikzpicture}[scale=0.93]       % STEP 0
    \node[draw, fill=orange] at (4, 2) (start) {$\alpha$};
    \node[draw,fill=Orchid,text=white] at (2, 2) (K) {$\beta$};
    \node[draw,fill=red,text=white] at (0, 2) (R) {$\gamma$};
    \node[draw,fill=LimeGreen,text=white] at (0, 0) (G) {$\epsilon$};
...
% not hot    \draw (G) edge[->] node[left] {$B$} (R);
    ...
    \node[anchor=west] at (6.5, 1) {$\beta \xrightarrow{B} \gamma \xleftarrow{B} \epsilon$};
  ...
  \begin{tikzpicture}[scale=0.93]      % STEP 1
    \node[draw, fill=orange] at (4, 2) (start) {$\alpha$};
    \node[draw,fill=Orchid,text=white] at (2, 2) (K) {$\beta$};
    \node[draw,fill=red,text=white] at (0, 2) (R) {$\gamma$};
    \node[draw,semitransparent,fill=LimeGreen,text=white] at (0, 0) (G) {$\epsilon$};
    \node[draw,fill=Aquamarine,text=white] at (2, 0) (B) {$\delta$};
    ...
    % labels à droite
    \node[anchor=west] at (6, 2) {\texttt{pending}:};
    \node[anchor=west, lightgray] at (6.5, 1) {$\beta \xrightarrow{B} \gamma \xleftarrow{B} \epsilon$};
    \node[anchor=west] at (6.5, 1.5) {$\gamma \xrightarrow{C} \delta \xleftarrow{C} \delta$};
  \end{tikzpicture}
  ...
  \begin{tikzpicture}[scale=0.93]      % STEP 2
    \node[draw, fill=orange] at (4, 2) (start) {$\alpha$};
    \node[draw,fill=Orchid,text=white] at (2, 2) (K) {$\beta$};
    \node[draw,fill=red,text=white] at (0, 2) (R) {$\gamma$};
    \node[draw,semitransparent,fill=LimeGreen,text=white] at (0, 0) (G) {$\epsilon$};
    ...
    \node[anchor=west] at (6.5, 0.5) {$\epsilon \xrightarrow{D} \gamma \xleftarrow{D} \gamma$};
  \end{tikzpicture}
  ...
  \begin{tikzpicture}[scale=0.93]      % STEP 3
    \node[draw, fill=orange] at (4, 2) (start) {$\alpha$};
    ...
    % labels à droite
    \node[anchor=west] at (6, 2) {\texttt{pending}:};
    \node[anchor=west, lightgray] at (6.5, 1.5) {$\beta \xrightarrow{B} \gamma \xleftarrow{B} \epsilon$};
    \node[anchor=west, lightgray] at (6.5, 1) {$\gamma \xrightarrow{C} \delta \xleftarrow{C} \delta$};
    \node[anchor=west, lightgray] at (6.5, 0.5) {$\epsilon \xrightarrow{D} \gamma \xleftarrow{D} \gamma$};
  ...
  \begin{tikzpicture}[scale=0.93]      % STEP 4
...
% $W$ is less than the number of atoms in the universe.
So, the final complexity of \textsc{CollapseMazeDense} is
$\bigO{BW + E \alpha(W)}$. If $E \approx W^2$ and $W \approx B$, then this gives
a nearly-quadratic algorithm, improving upon the known cubic one presented in
...
we also use an auxiliary array \texttt{hotpot} where $\texttt{hotpot}[\omega]$
...
\begin{algorithmic}[1]
  ...
  \Procedure{AddEdge'}{$\alpha, X, i, \beta$}
   ...
   \Else
   \State $\texttt{hot}[\beta, X] \gets \texttt{some} (\alpha, i)$   
   \State $\texttt{hotpot}[\beta] \gets X :: \texttt{hotpot}[\beta]$    \Comment{Register non-empty location}
   \EndIf
   \EndProcedure
  ...
  \For{each $X \in \texttt{hotpot}[\sigma]$}                          \Comment{sparse iteration}       
  \State $\texttt{some} (\omega, i) \gets \texttt{hot}[\sigma, X]$    \Comment{looking at $\omega \xrightarrow{X.i} \sigma$}
  \State $\Call{AddEdge'}{\omega, X, i, \pi}$                          \Comment{replace by $\omega \xrightarrow{X.i} \pi$}
  \EndFor  
  \EndProcedure
  ...
  \For{$0 \leq \alpha < W$}
    ...
    \State $\texttt{hotpot}[\alpha] \gets \texttt{[]}$                 
    \For{$0 \leq X < B$}
...
pointing to the smaller supernode (thanks to the \texttt{hotpot} array)---and
...
does a constant number of operations per iteration, excluding calls to
...
%   \node[draw,fill=LimeGreen,text=white]  at (2, -2) (G) {$\epsilon$};
...
The (very standard) proof is given in the full version. Therefore, the total
running time of \textsc{CollapseMazeSparse} is $\bigO{BW + E \log E}$. At first
glance, this could seem like a \emph{decremental} improvement compared to
\textsc{CollapseMazeDense}. However, the \emph{only} part that requires $BW$
operations is the initialization of the $\texttt{hot}$ array. We therefore see
two ways to cheat our way out of this embarrassing situation.
...
\item A good-natured reviewer suggested that ``\emph{the requirement that the
    array is initialized with \texttt{none} is not unreasonable at all. All you
    really need is the model of computation to provide \emph{any} uniform
    representation of the initial memory and then just call that your
    \texttt{none} value}''. We appreciate the empathy. But unlike this
  sickeningly well-meaning stance, we believe the reality is quite different: no
  existing hardware architecture that can run compiled C programs offers
  ``free'' memory initialization.
...
In the dual representation graph, an edge $\alpha \xrightarrow{X.i} \beta$ is
...
process even starts, we transform the dual representation graph into the
...
When the rewriting process is over, the spanning edge form (rooted) spanning trees
of the connected components. Therefore, when a pair of nodes are reachable by a
path, this path can be assembled by following the spanning edges. Compared to
Algorithms~\ref{algo:main-dense} and~\ref{algo:main-sparse}, this requires the
...
Checking emptiness of the intersection of \emph{two} context-free
...
of as follows: suppose you have an application to play recursive mazes\footnote{
Such as the one we designed, available at
  \url{https://github.com/orelcosseron/Fractal-Maze-Python}.}. In
...
\begin{figure}[tbh]
\begin{center}
  \includegraphics[width=0.45\textwidth]{screenshot_Alice.png}
  \hfill
  \includegraphics[width=0.45\textwidth]{screenshot_square.png}
\end{center}
...
\begin{center}
  ...
  \begin{tikzpicture}[scale=0.98]       % initial graph
...
\end{tikzpicture}
  ...
  \begin{tikzpicture}[scale=0.98]       % STEP 0
    \path[use as bounding box] (-2.5, -0.25) rectangle (11.5, 2.5);
...
\begin{scope}[xshift=-1.5cm,yshift=0.25cm]
    ...
    \node[anchor=west] at (10.5, 0.75) {$\beta \xrightarrow[\#2]{B} \gamma \xleftarrow[\#4]{B} \epsilon$};
...
\end{tikzpicture}
  ...
  \begin{tikzpicture}[scale=0.98]      % STEP 1
    \path[use as bounding box] (-2.5, -0.25) rectangle (11.5, 2.5);
...
\begin{scope}[xshift=-1.5cm,yshift=0.25cm]
  ...
  \end{scope}
    ...
    \node[anchor=west] at (10.5, 2) {\texttt{pending}:};
    \node[anchor=west] at (10.5, 1.5) {$\gamma \xrightarrow[\# 8]{C} \delta \xleftarrow[\# 6]{C} \delta$};
    \node[anchor=west,lightgray] at (10.5, 0.75) {$\beta \xrightarrow[\#2]{B} \gamma \xleftarrow[\#4]{B} \epsilon$};
\end{scope}
  \end{tikzpicture}
  ...
  \begin{tikzpicture}[scale=0.98]      % STEP 2 (new)
...
\end{tikzpicture}
  ...
  \begin{tikzpicture}[scale=0.98]      % STEP 3
    \path[use as bounding box] (-2.5, -0.25) rectangle (11.5, 3.125);
...
\end{tikzpicture}
  ...
  \begin{tikzpicture}[scale=0.98]      % STEP 4 (final)
